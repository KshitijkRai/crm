...
// Template is the representation of a parsed template. The *parse.Tree field is exported only
// for use by html/template and should be treated as unexported by all other clients.
...
func allUser() ([]user, error) {
	...
	for rows.Next() {
		...
		err := rows.Scan(&user.ID, &user.Firstname, &user.Lastname, &user.Email,
			&user.Password, &user.ProfilePic, &user.Active, &user.JoinedDate, &user.LastActive)
		if err != nil {
			return users, err
		}
...
func main() {
	...
	http.HandleFunc("/validateSignup", validateSignup)
	http.HandleFunc("/validateLogin", validateLogin)
	// Handle registers the handler for the given pattern in the DefaultServeMux. The documentation for
	// ServeMux explains how patterns are matched.
	// func http.Handle(pattern string, handler http.Handler)
	// stripPrefix returns a handler that serves HTTP requests by removing the given prefix from the request URL's
	// Path (and RawPath if set) and invoking the handler h.
...
func index(w http.ResponseWriter, r *http.Request) {
	...
	// ExecuteTemplate applies the template associated with t that has the given name to the
	// specified data object and writes the output to wr.
...
func upload(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == http.MethodPost {
		...
		// New returns a new hash.Hash computing the SHA1 checksum. The Hash also implements
		// encoding.BinaryMarshaler and encoding.BinaryUnmarshaler to marshal and unmarshal the internal state of the
		// hash.
...
func validateLogin(w http.ResponseWriter, r *http.Request) {
	...
	// QueryRow executes a query that is expected to return at most one row.
	// func (*sql.DB).QueryRow(query string, args ...interface{}) *sql.Row
	row := db.QueryRow("SELECT * FROM cms_user WHERE email = $1;", loginDetail.Email)
	var user user
	// Scan copies the columns from the matched row into the values pointed at by dest.
	// func (*sql.Row).Scan(dest ...interface{}) error
	err = row.Scan(&user.ID, &user.Firstname, &user.Lastname, &user.Email,
	...
	// Returns nil on success, or an error on failure.
	// func bcrypt.CompareHashAndPassword(hashedPassword []byte, password []byte) error
	err = bcrypt.CompareHashAndPassword([]byte(user.Password), []byte(loginDetail.Password))
	if err != nil {
